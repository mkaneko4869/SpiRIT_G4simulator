...
void SHistoManager::SetPrimaryVertex(G4PrimaryVertex* pv)
{
  ...
  for(G4int ipart=0; ipart<npart; ipart++){
    ...
    particle->SetTrackID(pp->GetTrackID());
    particle->SetParentID(0.);
...
void SHistoManager::SetSpiRITTrack(TrackerHitsCollection* THC)
{
  ...
  std::vector<G4double> trackTList; trackTList.clear(); // fastest hit timing for each track -> used to fill momentum of secondary particles
  
  for(int i=0; i<THC->entries(); i++){
    auto trackpoint = (TrackerHit*)THC->GetHit(i);
    auto info = trackpoint->GetTrackInformation();
    auto mom  = trackpoint->GetMomentum();
    auto etot = trackpoint->GetTotalEnergy();
    auto time = trackpoint->GetTime(); // global time
      
    if((info->GetOriginalParticle()->GetPDGEncoding()!=111)&&!(info->GetIsPrimary())) continue; // see only primary particle but children from pizero will be tracked ( pizero -> e+e-gamma decay)
    ...
    if(IsNewTrack){ // This hit generated by new track
      trackIDList.push_back(trackpoint->GetTrackID());
      trackTList.push_back(time);
      ...
      if(info->GetIsPrimary())currentTrack.SetFourMomentum(p.x(),p.y(),p.z(),info->GetOriginalEnergy());
      else currentTrack.SetFourMomentum(mom.x(),mom.y(),mom.z(),etot);
      currentTrack.SetIsPrimary(info->GetIsPrimary());
      currentTrack.SetTrackID( trackpoint->GetTrackID() );
      currentTrack.SetParentID( trackpoint->GetParentID() );
      atrack->SetTrack(currentTrack);
      atrack->AddStep(trackpoint->GetEdep(),trackpoint->GetStepLength());  
      nTrack++;
    }
    else{ // the parent track of this hit is already stored. continue filling hit as track component
      G4int iTrack=0; // get track list ID number
      ...
      atrack->AddStep(trackpoint->GetEdep(),trackpoint->GetStepLength());  
      if(!info->GetIsPrimary()) // for secondary, check if this hit is early or later than fastest hit out of stored. if early, fill momentum at that early time.
        if(time<trackTList.at(iTrack)){
          atrack->GetTrack()->SetFourMomentum(mom.x(),mom.y(),mom.z(),etot);
          trackTList.at(iTrack)=time;
        }
    }
  }
